...
# ===从hdf中读取1分钟数据
df = pd.read_hdf('eos_1min_data.h5', key='all_data')
# 选取某一时间段
...
rule_type = '5T'
# on 必须是时间列
# base 根据rule_type周期划分的开始偏移值 rule_type='5T' base=1 1分钟，6分钟
# lable 获取的时间是以时间段的开始还是结束为基准，left：开始时间（推荐），right：结束时间
# closed 数据边界是左包含还是右包含，left：左边不包含（推荐），right：右边不包含
...
period_df = period_df[['candle_begin_time', 'open', 'high', 'low', 'close', 'volume']]
print(period_df)
exit()
